          ...
          <div className="bg-card-app border border-card-border rounded-2xl shadow-xl w-full max-w-md overflow-hidden animate-fade-in">
            ...
            <form action={handleCreate} className="p-6 space-y-4">
              <div>
                ...
                >
                  <option value="" className="text-slate-900 bg-white">-- Lựa chọn độc giả --</option>
                  {readers.map(r => (
                    <option key={r.id} value={r.id} className="text-slate-900 bg-white">{r.name} - {r.phone}</option>
                  ))}
                </select>
              </div>
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full bg-white dark:bg-slate-800 border border-card-border rounded-xl p-3 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-orange-500 focus:border-transparent transition select-none cursor-pointer"
                >
                  <option value="" className="text-slate-900 bg-white">-- Lựa chọn sách --</option>
                  {books.map(b => (
                    <option key={b.id} value={b.id} disabled={b.quantity <= 0} className="text-slate-900 bg-white">
